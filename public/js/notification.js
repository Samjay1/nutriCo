        ...
        const readState = document.querySelectorAll('.readState');
        const viewState = document.querySelectorAll('.viewState');viewState
         

        notification_items.forEach((button,index)=>{
            button.addEventListener('click', ()=>{
            console.log('clicked', index, drop_content[index].className.split(' ')[0] )
            readState[index].innerHTML = ` <span class="px-3 py-1 bg-gray-300 rounded-2xl"></span>`
            
            if(drop_content[index].className.split(' ')[0] === 'truncate'){
                drop_content[index].className = 'drop-content'
               viewState[index].innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
